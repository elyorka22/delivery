...
export default function CourierDashboard() {
  ...
  const takeOrder = async (orderId: string) => {
    try {
      await api.post(`/courier/orders/${orderId}/take`);
      fetchOrders();
    } catch (error) {
  ...
  const updateOrderStatus = async (orderId: string, status: OrderStatus) => {
    try {
      await api.put(`/courier/orders/${orderId}/status`, { status });
      fetchOrders();
    } catch (error) {
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Панель курьера</h1>
        {loading ? (
            ...
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <div>
                    ...
                    {order.restaurant && (
                      <p className="text-gray-600">
                        Ресторан: {order.restaurant.name} - {order.restaurant.address}
                      </p>
                    )}
                  </div>
                  <span
                ...
                <div className="flex space-x-2">
                  {order.status === OrderStatus.READY && !order.courierId && (
                    <button
                  ...
                  {order.courierId && order.status === OrderStatus.READY && (
                    <button
